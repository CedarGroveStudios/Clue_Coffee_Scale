...
FONT_0 = bitmap_font.load_font("/fonts/helvB24.bdf")
FONT_1 = terminalio.FONT  # For graduated scale labels
...
zero_button_label = Label(FONT_1, text="Z", color=clue.RED, scale=2)
zero_button_label.x = 9
zero_button_label.y = 152
scale_group.append(zero_button_label)

# Place tick mark labels next to the graduated scale
for i in range(-1, 6):
    tick_value = Label(FONT_1, text=str(MAX_GR // 5 * i), color=clue.CYAN)
    if i == -1:
    ...
    tick_value.anchored_position = (98, 201 - (i * 40))
...
# Define the movable indicator bubble
...
while True:
    ...
    print(f" {grams_value.text} grams   {ounces_value.text} ounces")
    ...
    if clue.button_a:
        ...
        zero_button_circle.fill = clue.RED  # Set to red (stopped)
        ...
        zero_button_circle.fill = None  # Set to transparent (ready)
